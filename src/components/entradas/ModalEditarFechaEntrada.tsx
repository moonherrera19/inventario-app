...
interface ModalEditarFechaEntradaProps {
  open: boolean;
  onClose: () => void;
  entradaId: number;
  fechaActual: string;
...
export default function ModalEditarFechaEntrada({
  open,
  onClose,
  entradaId,
  fechaActual,
  refresh,
}: ModalEditarFechaEntradaProps) {
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-[#0f1217] border border-gray-700 rounded-lg p-6 w-full max-w-sm text-white">
        ...
        <input
          ...
          className="w-full px-3 py-2 bg-[#0f1217] border border-green-700 rounded text-white mb-4"
        />
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            onClick={guardar}
            disabled={loading}
            className="px-4 py-2 rounded bg-green-600 hover:bg-green-700 font-medium"
          >
